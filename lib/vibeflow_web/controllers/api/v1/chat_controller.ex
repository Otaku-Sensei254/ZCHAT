    ...
    case Vibeflow.Chat.get_conversation_by_uuid(uuid) do
      ...
      conversation ->
        if Vibeflow.Chat.member_of_conversation?(user, conversation.id) do
          ...
          other_member = Enum.find(conversation.conversation_members || [], fn m -> m.user_id != user.id end)
    ...
    if is_nil(message) do
      ...
      if message.user_id == user.id do
        case Vibeflow.Chat.update_message(message, %{content: content}) do
          ...
          {:error, _changeset} ->
            conn |> put_status(:unprocessable_entity) |> json(%{error: "Failed to update message"})
        end
      else
        conn |> put_status(:forbidden) |> json(%{error: "Not your message"})
      end
    ...
    case Vibeflow.Chat.get_conversation_by_uuid(uuid) do
      ...
      conversation ->
        case Enum.find(conversation.conversation_members, fn m -> m.user_id == user.id end) do
          nil -> conn |> put_status(:forbidden) |> json(%{error: "Not a member"})
          member ->
            case member |> Ecto.Changeset.change(%{message_skin: skin}) |> Vibeflow.Repo.update() do
              {:ok, _updated_member} ->
              ...
              {:error, _} ->
                conn |> put_status(:unprocessable_entity) |> json(%{error: "Failed to update skin"})
            end
        end
    end
  end
