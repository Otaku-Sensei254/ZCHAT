defmodule VibeflowWeb.Waves.WavesLive do
  ...
  # Add Logger to print terminal outputs
  require Logger
  ...
  def mount(_params, _session, socket) do
    {:ok,
     ...
     |> allow_upload(:media,
       ...
       max_entries: 1,
       # <--- RESTORED TO TRUE for smoother UX
       auto_upload: true,
  ...
  def render(assigns) do
    ...
    >
      ...
      <div class="absolute inset-0 z-0 bg-black">
        ...
        <% else %>
          <div id="camera-wrapper" class="w-full h-full relative">
            ...
            <div
              id="recording-timer"
              class="hidden absolute top-24 left-1/2 -translate-x-1/2 bg-red-600 px-4 py-1 rounded-full font-mono font-bold text-sm tracking-widest shadow-lg z-50 animate-pulse border-2 border-white/20"
            >
              00:00
            </div>
            <button
      ...
      <div class="absolute top-0 left-0 right-0 p-4 pt-8 z-40 bg-gradient-to-b from-black/60 to-transparent flex justify-between items-center pointer-events-none">
        <.link
          navigate={~p"/feed"}
          class="pointer-events-auto p-2 bg-black/20 backdrop-blur-md rounded-full hover:bg-black/40 transition"
        >
          <svg
        ...
        <%= if @uploads.media.entries != [] do %>
          <button
            ...
            class="pointer-events-auto text-xs font-bold bg-white/20 px-3 py-1.5 rounded-full backdrop-blur-md hover:bg-white/30 transition border border-white/10"
          >
            Retake
          </button>
        <% end %>
      ...
      <%= if @uploads.media.entries == [] do %>
        <div class="absolute bottom-0 left-0 right-0 z-30 pb-12 pt-24 bg-gradient-to-t from-black/90 via-black/40 to-transparent flex justify-center items-center gap-10">
          <button
            ...
            class="p-3 rounded-xl bg-white/10 backdrop-blur-md border border-white/20 hover:bg-white/20 transition"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
          ...
          <button
            id="btn-snap"
            class="w-20 h-20 rounded-full border-[6px] border-white bg-transparent active:scale-95 transition-all shadow-lg focus:outline-none"
          >
          </button>

          <button
            phx-click="switch_camera"
            class="p-3 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-white hover:bg-white/20 transition"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="w-6 h-6"
      ...
      <%= if @uploads.media.entries != [] and @step != 2 do %>
        <div class="absolute top-24 left-1/2 -translate-x-1/2 z-30 w-full flex justify-center">
          <%= if @selected_music do %>
            <div
              class="bg-black/50 backdrop-blur-md px-4 py-2 rounded-full flex items-center gap-2 border border-white/10 shadow-lg cursor-pointer animate-bounce-small"
              phx-click="open_music"
            >
          ...
          <% else %>
            <button
              phx-click="open_music"
              class="bg-black/40 backdrop-blur-md px-5 py-2 rounded-full flex items-center gap-2 border border-white/10 text-sm font-semibold hover:bg-black/60 transition"
            >
              🎵 Add Sound
            </button>
          <% end %>
        ...
        <div class="absolute bottom-0 left-0 right-0 p-6 pt-20 bg-gradient-to-t from-black/90 via-black/40 to-transparent z-30 flex flex-col gap-4">
          <form phx-submit="save_wave" phx-change="validate_upload">
            <input
              ...
              class="w-full bg-white/10 border border-white/10 rounded-xl text-white placeholder-gray-300 px-4 py-3 backdrop-blur-md focus:ring-2 focus:ring-blue-500 focus:bg-black/40 transition outline-none"
              autocomplete="off"
            />
            ...
            <div class="relative flex-1">
              <form phx-change="search_music" phx-submit="prevent_submit">
                <input
                  ...
                  class="w-full bg-zinc-800 text-white rounded-xl pl-4 pr-4 py-2.5 focus:ring-1 focus:ring-white border-none"
                  phx-debounce="500"
                  autoFocus
                />
              </form>
          ...
          <div class="flex-1 overflow-y-auto p-2 space-y-1">
            <%= for song <- @music_results do %>
              <div
                ...
                phx-value-id={song.track_id}
                class={"flex items-center gap-3 p-3 rounded-xl transition cursor-pointer " <> if(@selected_music && @selected_music.track_id == song.track_id, do: "bg-blue-600/20 border border-blue-500/50", else: "hover:bg-zinc-800")}
              >
                ...
                <div class="flex-1 min-w-0">
                  <p class={"font-bold truncate text-sm " <> if(@selected_music && @selected_music.track_id == song.track_id, do: "text-blue-400", else: "text-white")}>
                    {song.track_name}
                  </p>
      ...
      <%= if @selected_music do %>
        <audio
          src={@selected_music.preview_url}
          autoplay
          loop
          id="music-preview-player"
          class="hidden"
        >
        </audio>
      <% end %>
            ...
            [media | _] when is_map(media) ->
              ...
              case Vibeflow.Waves.create_wave(attrs) do
                {:ok, _} ->
                  ...
                  {:noreply,
                   socket |> put_flash(:info, "Shared!") |> push_navigate(to: ~p"/feed")}
  ...
  @impl true
  def handle_event("switch_camera", _, socket),
    do: {:noreply, push_event(socket, "switch-camera-mode", %{})}
