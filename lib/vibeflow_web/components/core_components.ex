defmodule VibeflowWeb.CoreComponents do
  @moduledoc """
  ...
  def icon(%{name: "hero-arrow-right-end-on-rectangle"} = assigns) do
    ~H"""
    <Heroicons.arrow_right_end_on_rectangle solid class={@class} />
    """
  end
        ...
        <div class="flex min-h-full items-center justify-center">
          <div class="w-full max-w-3xl p-4 sm:p-6 lg:py-8">
            <.focus_wrap
              ...
              class="hidden relative rounded-2xl bg-white p-6 sm:p-14 shadow-lg shadow-zinc-700/10 ring-1 ring-zinc-700/10 transition"
            >
              <div class="absolute top-4 right-4 sm:top-6 sm:right-5">
                <button
                  phx-click={hide_modal(@on_cancel, @id)}
                  type="button"
                  class="-m-3 flex-none p-3 opacity-20 hover:opacity-40"
                  aria-label={Gettext.dgettext(VibeflowWeb.Gettext, "default", "close")}
                >
  ...
  def simple_form(assigns) do
    ...
    <.form :let={f} for={@for} as={@as} {@rest}>
      <div class="space-y-8 bg-white dark:bg-transparent mt-10">
  ...
  def button(assigns) do
    ...
    <button
      type={@type}
      class={[
        "phx-submit-loading:opacity-75 rounded-lg bg-indigo-500 hover:bg-indigo-600 dark:bg-indigo-600 dark:hover:bg-indigo-700 py-2 px-3",
    ...
    <div phx-feedback-for={@name}>
      <label class="flex items-center gap-4 text-sm leading-6 text-zinc-600 dark:text-zinc-400">
        ...
        <input
          ...
          value="true"
          checked={@checked}
          class="rounded border-zinc-300 dark:border-zinc-600 text-indigo-500 dark:text-indigo-400 focus:ring-orange-500 dark:focus:ring-orange-400 dark:bg-dark-800"
          {@rest}
        />
        <%= @label %>
      </label>
    ...
    <div phx-feedback-for={@name}>
      ...
      <input
        ...
        class={[
          ...
          "dark:bg-dark-800 dark:text-white dark:border-zinc-600",
          "phx-no-feedback:border-zinc-300 phx-no-feedback:focus:border-zinc-400 phx-no-feedback:focus:ring-zinc-800/5",
          "dark:phx-no-feedback:focus:border-indigo-400",
          "border-zinc-300 focus:border-zinc-400 focus:ring-zinc-800/5 dark:focus:ring-orange-400/10",
          @errors != [] && "border-rose-400 focus:border-rose-400 focus:ring-rose-400/10"
        ]}
        {@rest}
      />
      ...
      <table class="mt-11 w-[40rem] sm:w-full">
        <thead class="text-left text-[0.8125rem] leading-6 text-zinc-500">
          <tr>
            ...
            <th class="relative p-0 pb-4"><span class="sr-only"><%= VibeflowWeb.Gettext.gettext("Actions") %></span></th>
          </tr>
        </thead>
        <tbody
          id={@id}
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(VibeflowWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(VibeflowWeb.Gettext, "errors", msg, opts)
    end
  end
  ...
  @doc """
  Generates initials from a user struct for avatar fallbacks.
  ...
  def render_markdown_mentions(text) do
    # Split the text into parts (plain text and @mentions)
    parts = String.split(text, ~r/(@[a-zA-Z0-9_]+)/, include_captures: true)
    ...
    <%= for part <- @parts do %>
      <%= if String.starts_with?(part, "@") do %>
        <% username = String.trim_leading(part, "@") %>
        <a href={~p"/users/#{username}"} class="text-indigo-400 font-bold hover:underline">
          <%= part %>
        </a>
      <% else %>
        <%= part %>
