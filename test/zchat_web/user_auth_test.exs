defmodule ZchatWeb.UserAuthTest do
  ...
  describe "log_in_user/3" do
    test "stores the user token in the session", %{conn: conn, user: user} do
      ...
      encoded_token = get_session(conn, :user_token)
      assert get_session(conn, :live_socket_id) == "users_sessions:#{encoded_token}"
      assert Accounts.get_user_by_session_token(Base.url_decode64!(encoded_token))
    end
  ...
  describe "fetch_current_user/2" do
    ...
    test "authenticates user from cookies", %{conn: conn, user: user} do
      ...
      cookie_encoded_token = logged_in_conn.cookies[@remember_me_cookie]
      ...
      assert conn.assigns.current_user.id == user.id
      assert get_session(conn, :user_token) == cookie_encoded_token
      assert get_session(conn, :live_socket_id) == "users_sessions:#{cookie_encoded_token}"
    end
  ...
  describe "on_mount :mount_current_user" do
    test "assigns current_user based on a valid user_token", %{conn: conn, user: user} do
      user_token = Accounts.generate_user_session_token(user)
      session = conn |> put_session(:user_token, Base.url_encode64(user_token)) |> get_session()
