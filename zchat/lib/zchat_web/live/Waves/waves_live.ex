defmodule ZchatWeb.Waves.WavesLive do
  ...
  # Make sure this Alias matches your actual Context file
  alias Zchat.Waves
  # If you are using Req for iTunes, make sure it's in mix.exs
...
@impl true
  def render(assigns) do
    ~H"""
    <div class="h-[100dvh] w-screen bg-black text-white flex flex-col overflow-hidden relative">

      <div class="absolute top-0 left-0 right-0 p-4 pt-6 flex justify-between items-center z-50 bg-gradient-to-b from-black/80 to-transparent">
        <.link navigate={~p"/feed"} class="text-white drop-shadow-md hover:scale-110 transition">
          ...
          <%= if @uploads.media.entries != [] do %>
            <div class="relative w-full h-full bg-black">
              ...
              <%= if @caption && @caption != "" do %>
                <div class="absolute left-4 bottom-28 z-40">
                  <div class="bg-black/40 px-3 py-1 rounded-md text-white text-sm max-w-[80vw] break-words"> <%= @caption %> </div>
                </div>
              <% end %>
            ...
            <%= if @mode == :camera do %>
              <div id="camera-wrapper" phx-hook="CameraCapture" class="w-full h-full relative">
                ...
                <div class="absolute bottom-0 left-0 right-0 h-40 bg-gradient-to-t from-black/90 to-transparent flex justify-center items-center gap-10 z-40">
                   ...
                   <button id="btn-snap" class="w-20 h-20 rounded-full border-4 border-white bg-white/20 hover:bg-white/40 transition-all active:scale-95 shadow-xl"></button>

                   <button id="btn-record" type="button" class="p-4 rounded-full bg-red-600 text-white hover:bg-red-500 backdrop-blur-md transition-all" title="Record/Stop">
                ...
                <div class="absolute left-4 bottom-36 z-50 w-[90%] max-w-md">
                  <form phx-change="update_caption" phx-submit="prevent_submit">
                    <input type="text" name="caption" value={@caption} placeholder="Add a caption..." class="w-full rounded-md bg-black/40 text-white placeholder-gray-300 px-3 py-2 text-sm focus:outline-none" />
                  </form>
                </div>
        ...
        <%= if @step == 3 do %>
          <div class="relative w-full h-full bg-black">
            ...
            <% end %>
              ...
              <%= if @caption && @caption != "" do %>
                <div class="absolute left-6 bottom-36 z-30">
                  <div class="bg-black/40 px-3 py-1 rounded-md text-white text-sm max-w-[80vw] break-words"> <%= @caption %> </div>
                </div>
              <% end %>
  ...
  def handle_event("trigger-capture", _, socket), do: {:noreply, socket}
  ...
  def handle_event("camera-error", %{"reason" => reason}, socket) do
    {:noreply, put_flash(socket, :error, "Camera error: #{reason}")}
  end

  def handle_event("cancel_upload", _, socket) do
    # Guard against empty entries if user spam clicks
    case socket.assigns.uploads.media.entries do
      [] -> {:noreply, socket}
      [entry | _] -> {:noreply, cancel_upload(socket, :media, entry.ref)}
    end
  end
  ...
  def handle_event("save_wave", _, socket) do
    # 1. Save File to Disk (Local dev setup)
    ...
    # 2. Build Attributes
    # Determine media type from file extension (basic heuristic)
    ext = Path.extname(media_url) |> String.downcase()
    media_type = if ext in [".mp4", ".mov", ".webm"], do: "video", else: "image"
